...
------------------------------------------- BODY ---------------------------------------------
-- Precondition: 
-- 1. disallowed_by_ccs_entities_off is omitted in PreloadedPT

-- Verification criteria: 
-- 1. SDL considers PreloadedPT as valid
-- 2. Start successfully
-- 3. Does not saved disallowed_by_ccs_entities_off in LPT

local test_case_name = "disallowed_by_ccs_entities_off_is_omitted_in_PreloadedPT"
...
-- Verify disallowed_by_ccs_entities_off is not included in Snapshot
function Test:VerifyDisallowedByCcsEntityIsNotInSnapShot()
  ...
  local file_name = ivsu_cache_folder.."/".."sdl_snapshot.json"
	...
	if item == nil then
    print ( " \27[32m disallowed_by_ccs_entities_off is not found in SnapShot \27[0m " )
		return true
	else
    print ( " \27[31m disallowed_by_ccs_entities_off is found in SnapShot \27[0m " )
		return false
	end
	file_json:close()
end
