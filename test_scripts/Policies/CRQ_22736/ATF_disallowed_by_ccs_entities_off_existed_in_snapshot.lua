...
------------------------------------------- BODY ---------------------------------------------
-- Precondition: disallowed_by_ccs_entities_off existed in PreloadedPT
...
local test_case_name = "TC_DisallowedByCcsEntitiesOnExistedInSnapshot"
...
function Test:VerifyDisallowedByCcsEntitiesOnInSnapShot()
	...
	if(file_name) then
		...
		if item == nil then
			self:FailTestCase("disallowed_by_ccs_entities_off is not found in SnapShot although is existed in PreloadedPT file")
			return false
		else
			print (" \27[32m disallowed_by_ccs_entities_off is found in SnapShot \27[0m ")
			return true
		end
		file_json:close()
	else
