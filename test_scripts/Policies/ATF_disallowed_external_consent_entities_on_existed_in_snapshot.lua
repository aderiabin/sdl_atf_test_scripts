...
------------------------------------------- BODY ---------------------------------------------
-- Precondition: disallowed_external_consent_entities_on existed in PreloadedPT
...
function Test:VerifyDisallowedByExternalConsentEntitiesOnInSnapShot()
  ...
  if(file_name) then
    ...
    if not item then
      self:FailTestCase("disallowed_by_external_consent_entities_on is not found in SnapShot although is existed in PreloadedPT file")
      return false
    else
      print (" \27[32m disallowed_by_external_consent_entities_on is found in SnapShot \27[0m ")
      return true
    end
    file_json:close()
  end
