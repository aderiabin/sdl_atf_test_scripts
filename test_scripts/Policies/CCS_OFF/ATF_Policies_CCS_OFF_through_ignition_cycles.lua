...
-- TEST 10:
-- In case
-- SDL has received SDL.OnAppPermissionConsent ("ccsStatus: OFF") from HMI
-- SDL must
-- use this value through ignition cycles
-- until this ccsStatus value is changed by corresponding notification from HMI.
--------------------------------------------------------------------------
-- Test 10.01:
-- Description: disallowed_by_ccs_entities_on/off . HMI -> SDL: OnAppPermissionConsent(ccsStatus OFF). Ignition Off then On.
...
--------------------------------------------------------------------------
Test[TEST_NAME_OFF .. "Precondition_HMI_sends_OnAppPermissionConsent_ccsStatus_OFF"] = function(self)
...
--------------------------------------------------------------------------
Test[TEST_NAME_OFF .. "Precondition_HMI_sends_OnAppPermissionConsent_ccsStatus_ON"] = function(self)
  hmi_app_id_1 = common_functions:GetHmiAppId(config.application1.registerAppInterfaceParams.appName, self)
  -- hmi side: sending SDL.OnAppPermissionConsent for applications
  self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", {
      appID = hmi_app_id_1, source = "GUI",
