...
-- 2.disallowed_by_external_consent_entities_on is omitted in PTU
-- Verification criteria:
-- 1. SDL considers this PTU as valid
-- 2. Does not saved disallowed_by_external_consent_entities_on in LocalPT
...
-- To make sure it does not contain dissallowed_external_consent_entity_on param
...
-- Verification criteria:
-- 1. SDL considers this PTU as valid
-- 2. Does not merge disallowed_by_external_consent_entities_on from PTU to LocalPT
...
Test[test_case_name .. "_HMI_sends_OnAppPermissionConsent_externalConsentStatus"] = function(self)
	...
	self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", {
		source = "GUI",
		externalConsentStatus = {{entityType = 120, entityID = 70, status = "OFF"}}
	})
...
-- UpdateTurnList ("Navigation-1") is assigned to default in ptu_without_dissallowed_external_consent_entity_on.json
-- Send UpdateTurnList to verify PTU success without disallowed_external_consent_entities_on in ptu file
function Test:UpdateTurnList_PositiveCase()
  local request = {
    turnList =
    {
