...
-- Description:
-- Two mobile applications with the same appIDs and vrSysnonyms and different appNames, are registering from different
...
-- 2)Mobile №2 sends RegisterAppInterface request (with all mandatories) with appID = 1, appName = "Test Application 2"
--    and vrSynonyms = "vrApp" to SDL
--   Check:
--    SDL sends RegisterAppInterface(resultCode = SUCCESS) response to Mobile №2
--    SDL sends OnAppRegistered(application.appName = "Test Application 2", vrSysnonyms = "vrApp") notification to HMI
---------------------------------------------------------------------------------------------------
...
local appParams = {
  ...
  [2] = { appName = "Test Application 2", appID = "0001", fullAppID = "0000001", vrSynonyms = {"vrApp"} }
}
