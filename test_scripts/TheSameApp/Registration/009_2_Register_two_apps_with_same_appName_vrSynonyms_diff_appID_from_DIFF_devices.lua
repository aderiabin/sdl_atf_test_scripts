...
-- Description:
-- Two mobile applications with the same appNames and same vrSysnonyms and different appID are registering from
-- different mobile devices. Check if there was sent an OnAppRegistered notification containing the same vrSysnonyms
-- field for both applications
--
-- Precondition:
...
--
-- Steps:
-- 1)Mobile №1 sends RegisterAppInterface request (with all mandatories) with appID = 0001,
--    appName = "Test Application" and vrSynonyms = "vrApp" to SDL
--   Check:
...
-- 2)Mobile №2 sends RegisterAppInterface request (with all mandatories) with appID = 00022,
--    appName = "Test Application" and vrSynonyms = "vrApp" to SDL
--   Check:
...
local appParams = {
  ...
  [2] = { appName = "Test Application", appID = "00022", fullAppID = "00000022", vrSynonyms = {"vrApp"} }
}
