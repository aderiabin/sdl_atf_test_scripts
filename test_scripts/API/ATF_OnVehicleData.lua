...
local permission_lines_subscribe_vehicle_data =
...
local permission_lines_on_vehicle_data =
[[ "OnVehicleData": {
  "hmi_levels": [
  "BACKGROUND",
  "FULL",
...
local svd_values = { gps = "VEHICLEDATA_GPS",
...
function Test:subscribeVehicleDataSuccess(params_send)
  local function SetSvdRequest(params_send)
    local temp = {}
  ...
  local function SetSvdResponse(params_send, vehicle_data_result_code)
    ...
    for i = 1, #params_send do
      if params_send[i] == "clusterModeStatus" then
        temp["clusterModes"] = {
          resultCode = vehicle_data_result_code_value,
          dataType = svd_values[params_send[i]]
        }
      else
        temp[params_send[i]] = {
          resultCode = vehicle_data_result_code_value,
...
--1. verify_SUCCESS_Notification_Case(notification,expect_notification)
...
function Test:verify_SUCCESS_Notification_Case(notification, expect_notification)
  local exp_notification
  if expect_notification == nil then
    exp_notification = notification
  else
...
function Test:SubscribeVehicleData_Positive()
  self:subscribeVehicleDataSuccess(all_vehicle_data)
end
...
--26. fuelRange: type=Float minvalue=0 maxvalue=10000 mandatory="false"
...
local function OnVehicleDataAllParametersLowerBoundSucess(TestCaseName)
...
local function common_Test_Cases_For_Notification()
  ...
  Test["OnVehicleData_AllParametersUpperBound_SUCCESS"] = function(self)
    local notification =
    {
      ...
      tirePressureValue = {
        ...
        rightRear = 65533,
        innerLeftRear = 65533,
        innerRightRear = 65533,
        frontRecommended = 65533,
        rearRecommended = 65533
      },
      tpms = "SENSOR_FAULT",
      turnSignal = "OFF"
    }
  ...
  for ParameterName,value in pairs(test_data) do
    Test["OnVehicleData_OnlyMandatoryParameters_SUCCESS_tirePressure_" .. tostring(ParameterName)] = function(self)
      local tire_pressure_value = {}
      tire_pressure_value[ParameterName] = value
      local notification = {tirePressure = tire_pressure_value}
      self:verify_SUCCESS_Notification_Case(notification)
    end
  end
  -----------------------------------------------------------------------------------------
...
local function VerifyECallInfoParameter()
...
VerifyECallInfoParameter()

local function VerifyEmergencyEventParameter()
...
VerifyEmergencyEventParameter()

local function VerifyBodyInformationParameter()
...
VerifyBodyInformationParameter()

local function VerifyClusterModeStatusParameter()
...
VerifyClusterModeStatusParameter()
...
local function VerifyAirbagStatusParameter()
...
--26. fuelRange: type="Float" minvalue="0" maxvalue="10000" mandatory="false"
local notification = {rpm = 1}
floatParameterInNotification:verify_Float_Parameter(notification, {"fuelRange"}, {0.000000, 10000.000000}, false)

--27. abs_State: type="ABS_STATE" mandatory="false"
local notification = {rpm =1, abs_State = {}}
enumParameterInNotification:verify_Enumeration_Parameter(notification,{"abs_State"},abs_state_values,false)
...
local param_boundary = {{0,65533}, {0,65533}, {0,65533}, {0,65533}, {0,65533}, {0,65533}, {0,65533}, {0,65533}}

for i = 1, #tire_pressure_value_params do
  integerParameterInNotification:verify_Integer_Parameter(notification, {"tirePressureValue", tire_pressure_value_params[i]}, param_boundary[i], false)
end
--29. tpms: type="TPMS" mandatory="false"
local notification = {rpm =1, tpms = {}}
enumParameterInNotification:verify_Enumeration_Parameter(notification,{"tpms"},tpms_values,false)
--30. turnSignal: type="TurnSignal" mandatory="false"
local notification = {rpm =1, turnSignal ={}}
enumParameterInNotification:verify_Enumeration_Parameter(notification,{"turnSignal"},turn_signal_values,false)
----------------------------------------------------------------------------------------------
-----------------------------------------TEST BLOCK IV----------------------------------------
...
local function SpecialNotificationChecks()
  ...
  ----------------------------------------------------------------------------------------------
  function Test:OnVehicleDataSeveralNotificationsWithTheSameValues()
    --hmi side: sending OnVehicleData notification
...
--Description: TC's checks SDL behavior by processing different request sequence with timeout with emulating of user's actions
...
local function SequenceChecks()
  ...
  local pt_name = testCasesForPolicyTable:createPolicyTableWithoutAPI("OnVehicleData")
  --Precondition: Update policy table
  testCasesForPolicyTable:updatePolicy(pt_name)
  --Send notification and check it is ignored
  function Test:OnVehicleDataIsNotExistInPTDisallowed()
    commonTestCases:DelayedExp(1000)
  ...
  ----------------------------------------------------------------------------------------------
  --Precondition: Build policy table file
  local permission_lines_on_vehicle_data_not_allow_parameters =
  [[ "OnVehicleData": {
    "hmi_levels": [
    "BACKGROUND",
    "FULL",
  ...
  ----------------------------------------------------------------------------------------------
  --Precondition: Build policy table file
  local permission_lines_for_base_4 = permission_lines_subscribe_vehicle_data .. ", \n"
  local permission_lines_for_group_1 = permission_lines_on_vehicle_data .. "\n"
...
local function DifferentHMIlevelChecks()
  ...
  function Test:OnVehicleDataNotificationInNoneHmiLevel()
    ...
    --hmi side: send notification
    self.hmiConnection:SendNotification("VehicleInfo.OnVehicleData", notification_all_parameters_low_bound)
    --mobile side: expected Notification
    EXPECT_NOTIFICATION("OnVehicleData", {})
    :Times(0)
  end
  ...
  if commonFunctions:isMediaApp() then
    ...
    OnVehicleDataAllParametersLowerBoundSucess("OnVehicleData_Notification_InLimitedHmiLevel")
    --Postcondition: Activate app
    commonSteps:ActivationApp()
  end
  ...
  OnVehicleDataAllParametersLowerBoundSucess("OnVehicleData_Notification_InBackgroundHmiLevel")
  ----------------------------------------------------------------------------------------------
end
...
local function Test_Block_VIII()
  ...
  -- Common variables
  local on_vehicle_data_full_notification =
  {
    speed = 0.0,
    fuelLevel = -6.000000,
    instantFuelConsumption = 0.000000,
  ...
  local function CRQ2_APPLINK_23963()
    ...
    local function OnVehicleDataParametersIsEmptyCreatePT()
      local path_to_pt = "user_modules/shared_testcases/PolicyTables/TestingPolicyTable_OnVehicleData_23963.json"
      os.execute('cp ' .. config.pathToSDL .. 'sdl_preloaded_pt.json' .. ' ' .. path_to_pt)
      local file = io.open(path_to_pt, "r")
