...
-- Specific case: RegisterAppInterface(keyboardProperties, WARNINGS) and default HMI level is FULL, LIMITED, BACKGROUND
...
local hmi_levels = {"FULL", "LIMITED", "BACKGROUND"}
for i = 1, #hmi_levels do
  common_steps:AddNewTestCasesGroup("Check timer 10s is started when RegisterAppInterface responds WARNINGS and default HMI level is " .. hmi_levels[i])
  Test["Precondition_Update_sdl_preloaded_pt.json_default_hmi_" .. hmi_levels[i]] = function (self)
  ...
  Test["RegisterAppInterface_WARNINGS_" .. hmi_levels[i]] = function(self)
    local app = config.application1.registerAppInterfaceParams
    app.appHMIType = {"MEDIA"}
  ...
  Test["UI.SGP with default keyboardProperties in 10s in case RegisterAppInterface_WARNINGS"] = function(self)
