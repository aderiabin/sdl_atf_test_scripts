...
Test[TEST_NAME .. "Precondition_HMI_sends_OnAppPermissionConsent"] = function(self)
  ...
  hmi_app_id_2 = common_multi_mobile_connections:GetHmiAppId(config.application2.registerAppInterfaceParams.appName, self)  
	...
	self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", 
   {ccsStatus = {{entityType = 127, entityID = 127, status = "OFF"}}, appID = hmi_app_id_2, consentedFunctions = nil, source = "GUI"})   
end
--]]
--------------------------------------------------------------------------
-- Precondition:
...
Test[TEST_NAME .. "Precondition_Emulate_ccsStatus_added_into_database"] = function(self)
  ...
  -- insert ccsStatus = {entityType = 127, entityID = 127, status = "OFF"}
  sql_query = "insert into _internal_ccs_status (entity_type, entity_id, on_off) values (127,127,'OFF'); "
  ful_sql_query = "sqlite3 " .. policy_file_temp .. " \"" .. sql_query .. "\""
  handler = io.popen(ful_sql_query, 'w')
  handler:close()
  -- insert ccsStatus = {entityType = 127, entityID = 1, status = "OFF"}
  sql_query = "insert into _internal_ccs_status (entity_type, entity_id, on_off) values (127,1,'OFF'); "
  ful_sql_query = "sqlite3 " .. policy_file_temp .. " \"" .. sql_query .. "\""
  handler = io.popen(ful_sql_query, 'w')
  handler:close()
  -- insert ccsStatus = {entityType = 1, entityID = 127, status = "OFF"}
  sql_query = "insert into _internal_ccs_status (entity_type, entity_id, on_off) values (1,127,'OFF'); "
  ful_sql_query = "sqlite3 " .. policy_file_temp .. " \"" .. sql_query .. "\""
  handler = io.popen(ful_sql_query, 'w')
  handler:close()
  os.execute("sleep 1")  
...
Test[TEST_NAME .. "MainCheck_ccsStatus_is_OFF_&_GetListOfPermissions_with_appID"] = function(self)
  ...
  :ValidIf(function(_,data)
    ...
    validate1 = common_functions_ccs_informing_hmi:Validate_ccsStatus_EntityType_EntityId(data, 127, 127)
