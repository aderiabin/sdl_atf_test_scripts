...
-- Specific case: RegisterAppInterface(keyboardProperties, SUCCESS) and default HMI level is FULL, LIMITED, BACKGROUND
...
for i = 1, #hmi_levels do
  common_steps:AddNewTestCasesGroup("Check timer 10s is started when default HMI level is " .. hmi_levels[i])
  ...
  Test["RegisterAppInterface_default_hmi_level_" .. hmi_levels[i]] = function(self)
    local app = config.application1.registerAppInterfaceParams
  ...
  Test["UI.SGP with default keyboardProperties in 10 seconds in " .. hmi_levels[i] .. " hmi level"] = function(self)
