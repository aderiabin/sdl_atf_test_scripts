--Requirements: [APPLINK-20706]:[RegisterAppInterface]: App registers at the first ignition
--cycle with languages matches to default ones

--Description: In case mobile app registers at the very first ignition cycle (or after Master Reset)
--and SDL does NOT receive at VR and TTS GetLanguage_response from HMI (SDL`s watchdog timeout
--is NOT expired yet) and UI, VR and TTS languages from RegisterAppInterface matches to
--default languages from 'HMI_capabilities.json' file SDL must: respond RegisterAppInterface
--(SUCCESS) to mobile app

-- Preconditions:
-- 1.Run  PreconditionSteps, which include next function: StartSDL and InitHMI 
-- 2.No response from HMI to SDL for VR.GetLanguage and TTS.GetLanguage

--Performed steps:
--1. Mob -> SDL: Application with appID is registering on SDL; request data satisfies the conditions for
--successfull registration. Language from RegisterAppInterface mathes to UI language

-- Expected result:
-- 1. SDL --> Mobile: ResgisterAppInterface(SUCCESS, success:true); Also SDL notifies HMI about successful
-- registration
-- ------------------------------------------Required Resources---------------------------------
...
--Redefine InitHMI_onReady() in order to comment VR.GetLanguage and TTS.GetLanguage,
--so HMI has only language for UI part.
...
common_steps:AddMobileConnection("Preconditions_AddMobileConnection")
common_steps:AddMobileSession("Preconditions_AddMobileSession")
-- -----------------------------------------------Body------------------------------------------
