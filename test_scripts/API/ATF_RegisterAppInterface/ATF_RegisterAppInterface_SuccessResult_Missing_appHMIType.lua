...
-- Description:
-- In case:
--  SDL receives RegisterAppInterface RPC from mobile app without appHMIType(s), SDL must:
...
-- Steps:
-- 1. Mob -> SDL: SendRPC RegisterAppInterface(appID = "testApp")
...
---------------------------------------------------------------------------------------------
--[[ Test ]]
function Test:RegisterAppInterface_without_appHMIType()
  local msg = {
    syncMsgVersion =
    {
      majorVersion = 2,
