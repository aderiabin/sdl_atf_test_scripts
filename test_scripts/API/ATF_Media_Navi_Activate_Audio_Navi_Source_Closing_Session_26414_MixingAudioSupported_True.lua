...
-- Case 1: Check Navi app is resumed to FULL Audible when there is activate audio source. Value of MixingAudioSupported is true. Reason off is closing session
-- Case 2: Check Navi app is resumed to LIMITED Audible when there is activate audio source. Value of MixingAudioSupported is true. Reason off is closing session
...
---------------------------------------------------------------------------------------------
-- Case 1: App is resumed to FULL HMI level in case closing session and MixingAudioSupported = true
---------------------------------------------------------------------------------------------
local test_case_name = "Case_1"
local full_test_case_name = "Case 1: Closing session and create session again, Navi app is resumed to FULL, Audible in case audio source is activate. MixingAudioSupported = true."
...
---------------------------------------------------------------------------------------------
-- Case 2: App is resumed to LIMITED HMI level in case closing session and MixingAudioSupported = true
---------------------------------------------------------------------------------------------
local test_case_name = "Case_2"
local full_test_case_name = "Case 2: Closing session and create session again, Navi app is resumed to LIMITED, Audible in case audio source is activate. MixingAudioSupported = true."
-- 1. Print new line to separate new test cases group
common_functions:newTestCasesGroup(full_test_case_name)
-- 2. Change app to Navi app
function_crq_26401:ChangeAppType(test_case_name,{"NAVIGATION"},false)
