...
function Test:Verify_App_Change_To_BACKGROUND_Incase_HmiLevel_Is_FULLorLIMITED_And_DeactiveHmi_Is_True()
  ...
  self[mobile_sessions[1]]:ExpectNotification("OnHMIStatus",{hmiLevel="BACKGROUND", audioStreamingState="NOT_AUDIBLE", systemContext = "MAIN"})
  self[mobile_sessions[2]]:ExpectNotification("OnHMIStatus",{hmiLevel="BACKGROUND", audioStreamingState="NOT_AUDIBLE", systemContext = "MAIN"})
end

function Test:Verify_App_Change_To_FULLorLIMITED_Incase_HmiLevel_Is_FULLorLIMITED_And_DeactiveHmi_Is_True()
  self.hmiConnection:SendNotification("BasicCommunication.OnEventChanged",{isActive= false, eventName="DEACTIVATE_HMI"})
  self[mobile_sessions[1]]:ExpectNotification("OnHMIStatus",{hmiLevel="LIMITED", audioStreamingState="AUDIBLE", systemContext = "MAIN"})
  self[mobile_sessions[2]]:ExpectNotification("OnHMIStatus",{hmiLevel="FULL", audioStreamingState="NOT_AUDIBLE", systemContext = "MAIN"})
end
