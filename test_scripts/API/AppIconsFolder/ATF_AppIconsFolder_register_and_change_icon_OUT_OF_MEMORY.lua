...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "OUT_OF_MEMORY")
...
function Test:SetAppIcon_action_bmp_OUT_OF_MEMORY()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
	...
	:Do(function(_,data)
    self.hmiConnection:SendError(data.id, data.method, "OUT_OF_MEMORY", "Error message.")
  end)
  self.mobileSession:ExpectResponse(cid, 
      {success = false, resultCode = "OUT_OF_MEMORY", info = "Error message."})
end
