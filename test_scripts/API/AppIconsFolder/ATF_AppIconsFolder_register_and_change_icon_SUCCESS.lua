...
-- 13. HMI->SDL:SetAppIcon(success = true, resultCode = "SUCCESS")
-- 14. Get file size of icon
-- Expected Result: File size is different with step #7
------------------------------------------------------------------------------------------------------
...
function Test:Get_Icon_File_Size_To_Check_File_Replaced_In_Next_Steps()
  file_size_1 = GetFileSize(app_icons_folder_path .. "/" .. app_params.appID)
...
function Test:SetAppIcon_action_bmp_SUCCESS()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
...
function Test:Check_Icon_Replaced_By_Compare_File_Size()
  file_size_2 = GetFileSize(app_icons_folder_path .. "/" .. app_params.appID)
  common_functions:UserPrint(32, "Icon file size: " .. tostring(file_size_2))      
  if file_size_2 == file_size_1 then
    self:FailTestCase("Application's icon in AppIconsFolder is not changed.")
  end
end
