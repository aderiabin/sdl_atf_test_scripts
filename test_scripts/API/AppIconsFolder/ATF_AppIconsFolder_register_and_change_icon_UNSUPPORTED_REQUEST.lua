...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "UNSUPPORTED_REQUEST")
...
function Test:SetAppIcon_action_bmp_UNSUPPORTED_REQUEST()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
	...
	:Do(function(_,data)
    self.hmiConnection:SendError(data.id, data.method, "UNSUPPORTED_REQUEST", "Error message.")
  end)
  self.mobileSession:ExpectResponse(cid, 
      {success = false, resultCode = "UNSUPPORTED_REQUEST", info = "Error message."})
end
