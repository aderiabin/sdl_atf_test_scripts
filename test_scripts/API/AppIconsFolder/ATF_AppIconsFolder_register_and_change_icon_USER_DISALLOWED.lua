...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "USER_DISALLOWED")
...
function Test:SetAppIcon_action_bmp_USER_DISALLOWED()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
	...
	:Do(function(_,data)
    self.hmiConnection:SendError(data.id, data.method, "USER_DISALLOWED", "Error message.")
  end)
  self.mobileSession:ExpectResponse(cid, 
      {success = false, resultCode = "USER_DISALLOWED", info = "Error message."})
end
