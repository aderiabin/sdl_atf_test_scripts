...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "TOO_MANY_PENDING_REQUESTS")
...
function Test:SetAppIcon_action_bmp_TOO_MANY_PENDING_REQUESTS()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
    ...
    self.hmiConnection:SendError(
        data.id, data.method, "TOO_MANY_PENDING_REQUESTS", "Error message.")
  end)
  self.mobileSession:ExpectResponse(cid, 
      {success = false, resultCode = "TOO_MANY_PENDING_REQUESTS", info = "Error message."})
end
