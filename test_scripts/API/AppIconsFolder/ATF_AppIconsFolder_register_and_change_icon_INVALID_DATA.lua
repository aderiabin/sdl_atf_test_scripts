...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "INVALID_DATA")
...
function Test:SetAppIcon_action_bmp_INVALID_DATA()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
	...
	:Do(function(_,data)
    self.hmiConnection:SendError(data.id, data.method, "INVALID_DATA", "Error message.")
  end)
  self.mobileSession:ExpectResponse(cid, 
      {success = false, resultCode = "INVALID_DATA", info = "Error message."})
end
