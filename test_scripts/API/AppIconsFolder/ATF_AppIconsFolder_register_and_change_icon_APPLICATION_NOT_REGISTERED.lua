...
-- 13. HMI->SDL:SetAppIcon(success = false, resultCode = "APPLICATION_NOT_REGISTERED")
...
function Test:SetAppIcon_action_bmp_APPLICATION_NOT_REGISTERED()
  local cid = self.mobileSession:SendRPC("SetAppIcon",{
      syncFileName = "action.bmp"})
  EXPECT_HMICALL("UI.SetAppIcon",{
    syncFileName = {
    ...
    self.hmiConnection:SendError(
        data.id, data.method, "APPLICATION_NOT_REGISTERED", "Error message.")
  end)
