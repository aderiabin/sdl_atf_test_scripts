...
-- Description:
-- SDL will send value BACK of UI.GetCapabilities.hmiZoneCapabilities from HMI to mobile

-- Preconditions:
-- 1. HMI -> SDL: value BACK of UI.GetCapabilities.hmiZoneCapabilities

-- Steps:
-- 1. Register App

-- Expected result:
-- SDL -> Mob: value BACK of UI.GetCapabilities.hmiZoneCapabilities
---------------------------------------------------------------------------------------------
...
function Test:Preconditions_InitHMI_OnReady()
  ...
  :Do(function(_,data)
      send_hmi_capabilities.hmiZoneCapabilities = "BACK"
...
function Test:Register_App()
  ...
  EXPECT_RESPONSE(cid, {success = true, resultCode = "SUCCESS", hmiZoneCapabilities = {"BACK"}})
