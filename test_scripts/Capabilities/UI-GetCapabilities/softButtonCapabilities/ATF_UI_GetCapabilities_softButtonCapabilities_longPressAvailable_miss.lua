...
-- Description:
-- SDL will use default capabilities stored at hmi_capabilities.json file when value softButtonCapabilities.longPressAvailable is missed

-- Preconditions:
-- 1. HMI -> SDL: value of softButtonCapabilities.longPressAvailable in UI.GetCapabilities is missed
...
function Test:Register_App()
  ...
  EXPECT_RESPONSE(cid, {success = true, resultCode = "SUCCESS", softButtonCapabilities =
      {{
          shortPressAvailable = false,
          longPressAvailable = softButtonCapabilities[1].shortPressAvailable,
          imageSupported = false,
          upDownAvailable = false}}})
