...
-- Description:
-- SDL will use default value of phoneCall when value hmiCapabilities.phoneCall is invalid

-- Preconditions:
-- 1. HMI -> SDL: invalid value of hmiCapabilities.phoneCall in UI.GetCapabilities

-- Steps:
-- 1. Register App

-- Expected result:
-- SDL -> Mob: value hmiCapabilities.phoneCall is true
---------------------------------------------------------------------------------------------
...
function Test:Preconditions_InitHMI_OnReady()
  ...
  :Do(function(_,data)
      send_hmi_capabilities.hmiCapabilities.phoneCall = "false"
...
function Test:Register_App()
  ...
  -- EXPECT_RESPONSE(cid, {success = true, resultCode = "SUCCESS", hmiCapabilities = {phoneCall = default.phoneCall, navigation = true}})
