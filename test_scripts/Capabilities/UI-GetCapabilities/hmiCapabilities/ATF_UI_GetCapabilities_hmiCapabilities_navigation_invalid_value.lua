...
-- Description:
-- SDL will use default value of navigation when value hmiCapabilities.navigation is invalid

-- Preconditions:
-- 1. HMI -> SDL: invalid value of hmiCapabilities.navigation in UI.GetCapabilities

-- Steps:
-- 1. Register App

-- Expected result:
-- SDL -> Mob: value hmiCapabilities.navigation is false
---------------------------------------------------------------------------------------------
...
function Test:Preconditions_InitHMI_OnReady()
  ...
  :Do(function(_,data)
      send_hmi_capabilities.hmiCapabilities.navigation = "false"
...
function Test:Register_App()
  ...
  -- EXPECT_RESPONSE(cid, {success = true, resultCode = "SUCCESS", hmiCapabilities = {navigation = default.navigation, phoneCall = false}})
