...
for i = 1, #speechCapabilities_list +1 do
  if (i==#speechCapabilities_list + 1) then
    ...
    common_steps:AddNewTestCasesGroup("Test case: HMI sends TTS.GetCapabilities with all speechCapabilities and all prerecordedSpeech")
  else
    SpeechCapabilities = {speechCapabilities_list[i]}
    PrerecordedSpeechCapabilities = {prerecordedSpeech_list[i]}
    common_steps:AddNewTestCasesGroup("Test case: HMI sends TTS.GetCapabilities with speechCapabilities = " ..SpeechCapabilities[1] .. " and prerecordedSpeech = " .. PrerecordedSpeechCapabilities[1])
  end
  ...
  function Test:MobileRegisterAppAndVerifyTTSCapabilities()
    ...
    EXPECT_RESPONSE(correlationId, { success = true, speechCapabilities = SpeechCapabilities,prerecordedSpeech = PrerecordedSpeechCapabilities})
    :Do(function(_,data)
      end)
  end
