...
function Test:resumptionSuccessWhenEmergencyEnded4Apps_FullL_Limited_Limited_Background()
	--Emergency ended
	self.hmiConnection:SendNotification("BasicCommunication.OnEventChanged",{isActive= false, eventName="EMERGENCY_EVENT"})	
...
local function Emergency_IsActive_And_Register4Apps(pos)
	if (pos==1) then		
		Test["Emergency_Is_Active_Before_Register_App"] = function(self)				
			userPrint(35, "=================== EMERGENCY is active ===================")
...
------------------------------------------------------------------------------------------------------------------------------------
-- Case: Single App (FULL) - Unexpected Disconnect: Emergency is active BEFORE app is connected and waiting for resume
-- Step1: Change App Type
-- Step2: Register/Activate app
-- Step3: Close/Start sesstion
-- Step4: EMERGENCY.True
-- Step5: Active app
...
------------------------------------------------------------------------------------------------------------------------------------
local function Resumption_HMIlevelFULL_EmergencyTrue_Before_UNEXPECTED_DISCONNECT()		
	for i =1, #test_data do
		commonFunctions:newTestCasesGroup("Single App (FULL) - Unexpected Disconnect: Emergency is active BEFORE app is connected and waiting for resume: " ..test_data[i].app)
		...
		Test[test_data[i].app .."_Emergency_Is_Active_Before_Register_App"] = function(self)
			self:emergencyIsActiveAndRegisterApp(1) --1:Emergency is active before app reconnected
		end
		...
		Test[test_data[i].app .."_Resumption_SingleApp_HMIlevelFULL_EmergencyIsActive_Before_AppIsConnected_Valid"] = function(self)
				self:resumptionSuccessWhenEmergencyEnded1App(hmi_level_FULL,test_data[i].appType) 
		end
		
	end	
end
Resumption_HMIlevelFULL_EmergencyTrue_Before_UNEXPECTED_DISCONNECT()

------------------------------------------------------------------------------------------------------------------------------------
-- Case: Single App (LIMITED) - Unexpected Disconnect: Emergency is active BEFORE app is connected and waiting for resume
-- Step1: Change App Type
...
-- Step9: Resumption success
------------------------------------------------------------------------------------------------------------------------------------
local function Resumption_HMIlevelLIMITED_EmergencyTrue_Before_UNEXPECTED_DISCONNECT()		
	for i =1, #test_data-1 do
		commonFunctions:newTestCasesGroup("Single App (LIMITED) - Unexpected Disconnect: Emergency is active BEFORE app is connected and waiting for resume:  " ..test_data[i].app)
		Test["Change_App_Type_To_" .. test_data[i].app] = function(self)
			userPrint(35, "================= Precondition ==================")
			self:change_App_Params(1,test_data[i].appType,test_data[i].isMedia)			
		end	
		...
		-- case: resumption successfully - isActive Valid
		Test[test_data[i].app .."_Resumption_SingleApp_HMIlevelLIMITED_EmergencyIsActive_Before_AppIsConnected_Valid"] = function(self)
				self:resumptionSuccessWhenEmergencyEnded1App(hmi_level_LIMITED,test_data[i].appType) 
		end
	end	
end
Resumption_HMIlevelLIMITED_EmergencyTrue_Before_UNEXPECTED_DISCONNECT()

------------------------------------------------------------------------------------------------------------------------------------
-- Case: Single App (FULL) - Unexpected Disconnect: Emergency is active AFTER app is connected and waiting for resume
-- Step1: Change App Type
-- Step2: Register/Activate app
-- Step3: Close/Start sesstion
-- Step4: EMERGENCY.True
...
-- Step9: Resumption success
------------------------------------------------------------------------------------------------------------------------------------
local function Resumption_HMIlevelFULL_EmergencyTrue_After_UNEXPECTED_DISCONNECT()		
	for i =1, #test_data do
		commonFunctions:newTestCasesGroup("Single App (FULL) - Unexpected Disconnect: Emergency is active AFTER app is connected and waiting for resume:  " ..test_data[i].app)
		...
		Test[test_data[i].app .."_Emergency_Is_Active_Before_Register_App"] = function(self)
			self:emergencyIsActiveAndRegisterApp(2) --2:Emergency is active after app reconnected
		end
		...
		-- case: resumption successfully - isActive Valid 
		Test[test_data[i].app .."_Resumption_SingleApp_HMIlevelFULL_EmergencyIsActive_After_AppIsConnected_Valid"] = function(self)
				self:resumptionSuccessWhenEmergencyEnded1App(hmi_level_FULL,test_data[i].appType) 
		end
		
	end	
end
Resumption_HMIlevelFULL_EmergencyTrue_After_UNEXPECTED_DISCONNECT()

------------------------------------------------------------------------------------------------------------------------------------
-- Case: Single App (LIMITED) - Unexpected Disconnect: Emergency is active AFTER app is connected and waiting for resume
-- Step1: Change App Type
-- Step2: Register/Activate app/ChangeHMIToLimited
-- Step3: Close/Start sesstion
-- Step4: EMERGENCY.True
...
local function Resumption_HMIlevelLIMITED_EmergencyTrue_After_UNEXPECTED_DISCONNECT()		
	for i =1, #test_data-1 do
		commonFunctions:newTestCasesGroup("Single App (LIMITED) - Unexpected Disconnect: Emergency is active AFTER app is connected and waiting for resume:  " ..test_data[i].app)
...
Resumption_HMIlevelLIMITED_EmergencyTrue_After_UNEXPECTED_DISCONNECT()
...
--///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////--
------------------------------------------------------------------------------------------------------------------------------------
-- Case: Multiple Apps (FULL,LIMITED,LIMITED,BACKGROUND) - Unexpected Disconnect: Emergency is active AFTER apps is connected and waiting for resume
...
local function Resumption_MultiApps_FULL_LIMITED_LIMITED_BACKGOUND_EmergencyIsActive_Before_AppIsConnected_UNEXPECTED_DISCONNECT()		
	commonFunctions:newTestCasesGroup("Multiple Apps (FULL,LIMITED,LIMITED,BACKGROUND) - Unexpected Disconnect: Emergency is active AFTER apps is connected and waiting for resume: ")
	...
	Test["Resumption_MultiApps_HMIlevelFULL_EmergencyIsActive_Before_AppIsConnected_IsActiveValid"] = function(self)
			self:resumptionSuccessWhenEmergencyEnded4Apps_FullL_Limited_Limited_Background() 
	end
end
Resumption_MultiApps_FULL_LIMITED_LIMITED_BACKGOUND_EmergencyIsActive_Before_AppIsConnected_UNEXPECTED_DISCONNECT()
...
local function Resumption_MultiApps_FULL_LIMITED_LIMITED_BACKGOUND_EmergencyIsActive_After_AppIsConnected_UNEXPECTED_DISCONNECT()		
	commonFunctions:newTestCasesGroup("Multiple Apps (FULL,LIMITED,LIMITED,BACKGROUND) - Unexpected Disconnect: Emergency is active AFTER apps is connected and waiting for resume: ")
...
Resumption_MultiApps_FULL_LIMITED_LIMITED_BACKGOUND_EmergencyIsActive_After_AppIsConnected_UNEXPECTED_DISCONNECT()
...
--///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////--
------------------------------------------------------------------------------------------------------------------------------------
-- Case: Multiple Apps (FULL,LIMITED,LIMITED,LIMITED) - Unexpected Disconnect: Emergency is active BEFORE apps is connected and waiting for resume
-- Step1: Unregister previous Apps
-- Step2: Register/Activate apps
-- Step3: Close/Start sesstion
-- Step4: EMERGENCY.True
...
-- Step9: Resumption success
------------------------------------------------------------------------------------------------------------------------------------
local function Resumption_MultiApps_FULL_LIMITED_LIMITED_LIMITED_EmergencyIsActive_Before_AppIsConnected_UNEXPECTED_DISCONNECT()		
	commonFunctions:newTestCasesGroup("Multiple Apps (FULL,LIMITED,LIMITED,LIMITED) - Unexpected Disconnect: Emergency is active BEFORE apps is connected and waiting for resume: ")
...
Resumption_MultiApps_FULL_LIMITED_LIMITED_LIMITED_EmergencyIsActive_Before_AppIsConnected_UNEXPECTED_DISCONNECT()

------------------------------------------------------------------------------------------------------------------------------------
-- Case: Multiple Apps (FULL,LIMITED,LIMITED,LIMITED) - Unexpected Disconnect: Emergency is active AFTER apps is connected and waiting for resume
-- Step1: Unregister previous Apps
-- Step2: Register/Activate apps
-- Step3: Close/Start sesstion
-- Step4: Active apps
-- Step5: EMERGENCY.True
-- Step6: Send isActive Invalid value (Empty, Wrong size, Is Missed)
-- Step7: Resumption postpone
-- Step8: Send IsActive Valid value (isActive=false)
-- Step9: Resumption success
------------------------------------------------------------------------------------------------------------------------------------
local function Resumption_MultiApps_FULL_LIMITED_LIMITED_LIMITED_EmergencyIsActive_After_AppIsConnected_UNEXPECTED_DISCONNECT()		
	commonFunctions:newTestCasesGroup("Multiple Apps (FULL,LIMITED,LIMITED,LIMITED) - Unexpected Disconnect: Emergency is active AFTER apps is connected and waiting for resume: ")
...
Resumption_MultiApps_FULL_LIMITED_LIMITED_LIMITED_EmergencyIsActive_After_AppIsConnected_UNEXPECTED_DISCONNECT()
