...
---------------------------------------------------------------------------------------------
function StartVR(test_case_name, event)
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Requirement summary: Resumption for multiple applications FULL/LIMITED/LIMITED/BACKGROUND) is postponed in case UNEXPECTED_DISCONNECT and VR is active BEFORE applications are connected
-- 1.Preconditions:
...
local function CheckMultipleAppsFullLimitedLimitedBackgroundArePostponedWhenVRIsStartedBeforeRegisteredApp()
  common_steps:AddNewTestCasesGroup("Multiple apps (Full-Limited-Limited-Background) are postponed in case UNEXPECTED_DISCONNECT and VR is active BEFORE apps are connected")
  -- Precondition
  for i = 1, #apps do
  ...
  for i = 1, #MOBILE_SESSION do
    common_steps:CloseMobileSession("TC_1_Close_Mobile_Session_" .. tostring(i), MOBILE_SESSION[i])
  end
  -- Body
  StartVR("TC_1_Start_VR")
  for i = #apps, 1, -1 do
    common_steps:AddMobileSession("TC_1_Add_Mobile_Session_" .. tostring(i), _, MOBILE_SESSION[i])
    common_steps:RegisterApplication("TC_1_Register_App_" .. apps[i].appName, MOBILE_SESSION[i], apps[i])
  end
  StopVRWithDelayTime("TC_1_Stop_VR", 10000)
  CheckAppsResumption("TC_1_Verify_Resumption_MultipleApps_Sucess_When_IsActive_Valid", expected_hmi_status_3apps)
  -- Post condition
  for i = 1, #apps do
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Requirement summary: Resumption for multiple applications FULL/LIMITED/LIMITED/BACKGROUND) is postponed in case UNEXPECTED_DISCONNECT and VR is active AFTER applications are connected
-- 1.Preconditions:
-- -- 1.1. Applications (NAVIGATION/COMMUNICATION/MEDIA/NON_MEDIA) are (FULL/LIMITED/LIMITED/BACKGROUND)
-- -- 1.2. Close session
-- 2.Steps:
...
local function CheckMultipleAppsFullLimitedLimitedBackgroundArePostponedWhenVRIsStartedAfterRegisteredApp()
  ...
  for i = 1, #MOBILE_SESSION do
    common_steps:CloseMobileSession("TC_2_Close_Mobile_Session_" .. tostring(i), MOBILE_SESSION[i])
  end
  --Body
  for i = #apps, 1, -1 do
    common_steps:AddMobileSession("TC_2_Add_Mobile_Session_" .. tostring(i), _, MOBILE_SESSION[i])
  ...
  StopVRWithDelayTime("TC_2_Stop_VR", 10000)
  CheckAppsResumption("TC_2_Verify_Resumption_MultipleApps_Sucess_When_IsActive_Valid", expected_hmi_status_3apps)
  -- Post condition
  for i = 1, #apps do
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Requirement summary: Resumption for multiple applications FULL/LIMITED/LIMITED/LIMITED) is postponed in case UNEXPECTED_DISCONNECT and VR is active BEFORE applications are connected
-- 1.Preconditions:
...
local function CheckMultipleAppsFullLimitedLimitedLimitedArePostponedWhenVRIsStartedBeforeRegisteredApp()
  common_steps:AddNewTestCasesGroup("Multiple apps (Full-Limited-Limited-Limited) are postponed in case UNEXPECTED_DISCONNECT and VR is active BEFORE apps are connected")
  -- Precondition
  for i = 1, #apps do
  ...
  for i = 1, #MOBILE_SESSION do
    common_steps:CloseMobileSession("TC_3_Close_Mobile_Session_" .. tostring(i), MOBILE_SESSION[i])
  end
  -- Body
  ...
  StopVRWithDelayTime("TC_3_Stop_VR", 10000)
  CheckAppsResumption("TC_3_Verify_Resumption_MultipleApps_Sucess_When_IsActive_Valid", expected_hmi_status_4apps)
  -- Post condition
  for i = 1, #apps do
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Requirement summary: Resumption for multiple applications FULL/LIMITED/LIMITED/LIMITED) is postponed in case UNEXPECTED_DISCONNECT and VR is active AFTER applications are connected
-- 1.Preconditions:
-- -- 1.1. Applications (NAVIGATION/COMMUNICATION/MEDIA/NON_MEDIA) are (FULL/LIMITED/LIMITED/LIMITED)
-- -- 1.2. Close session
-- 2.Steps:
...
local function CheckMultipleAppsFullLimitedLimitedLimitedArePostponedWhenVRIsStartedAfterRegisteredApp()
  ...
  for i = 1, #MOBILE_SESSION do
    common_steps:CloseMobileSession("TC_4_Close_Mobile_Session_" .. tostring(i), MOBILE_SESSION[i])
  end
  --Body
  for i = #apps, 1, -1 do
    common_steps:AddMobileSession("TC_4_Add_Mobile_Session_" .. tostring(i), _, MOBILE_SESSION[i])
  ...
  StopVRWithDelayTime("TC_4_Stop_VR", 10000)
  CheckAppsResumption("TC_4_Verify_Resumption_MultipleApps_Sucess_When_IsActive_Valid", expected_hmi_status_4apps)
  -- Post condition
  for i = 1, #apps do
