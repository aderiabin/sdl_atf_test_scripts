...
-- parent_item = {... ,"functional_groupings", "*", "rpc"}
        -- "rpcs": {}
------------------------------------------
-- function CommonFunctions:FindAndRemoveItemsOnJsonFile(json_file, parent_item, removed_item)
  -- local file = io.open(json_file, "r")
  -- local json_data = file:read("*all") -- may be abbreviated to "*a";
  -- file:close()
  -- local json = require("modules/json")
...
-- Remove items into json file
-- @param json_file: file name of a JSON file to be removed items
-- @param parent_item: it will be remove items, example {"abc" , "second level", "*", "rpc", "xyz"}
-- @param removed_items: it is a array of items will be removed
-----------------------------------------------------------------------------
function CommonFunctions:RemoveItemsFromJsonFile(json_file, parent_item, removed_items)
  local file = io.open(json_file, "r")
  ...
  local json = require("modules/json")
  local data = json.decode(json_data)
  CommonFunctions:RemoveItemsOnTable(data, parent_item, removed_items)
  data = json.encode(data)
  file = io.open(json_file, "w")
  file:write(data)
  file:close()
...
-----------------------------------------------------------------------------
-- Remove items into json file
-- @param json_table: a table is removed items
-- @param parent_item: full path to item that will be remove children items
-- @param removed_items: it is a array of items will be removed
-----------------------------------------------------------------------------
function CommonFunctions:RemoveItemsOnTable(json_table, parent_item, removed_items)
  -- Go to parent item
  local parent = json_table
  if #parent_item == 0 then
    -- Remove items
    for i = 1, #removed_items do
      print("XXXXXXXXXXXXXXXXXXXX")
      -- print(tostring(removed_items[i]))
      -- print(tostring(removed_items))
      CommonFunctions:PrintTable(parent[removed_items[i]])
      parent[removed_items[i]] = nil
      print("YYYYYYYYYYYYYYYYY")
       print(tostring(removed_items[i]))
    end
  else
    if parent_item[1] == "*" then 
      for k, v in pairs(parent) do
        local new_parent = {}
        for j = 2, #parent_item do
          table.insert(new_parent, parent_item[j])
        end
        CommonFunctions:RemoveItemsOnTable(json_table[k], new_parent, removed_items)
      end
    else
      local new_parent = {}
      for j = 2, #parent_item do
      ...
      CommonFunctions:RemoveItemsOnTable(json_table[parent_item[1]], new_parent, removed_items)
    end -- if parent_item[1] = "*" then 
